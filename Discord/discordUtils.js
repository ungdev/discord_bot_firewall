        ...
        .find(
          (channel) =>
            (channel.name
                .toLowerCase()
                .includes(
                  " " + channelToDelete.name.toLowerCase()
                ) ||
              channel.name
                .toLowerCase()
                .includes(
      ...
      (await msg.guild.roles.fetch()).cache
        .find(
          (role) =>
            ...
            channelToDelete.name.toUpperCase()
        )
        .delete("Demandé par " + msg.author.tag + " " + msg.author.username)
        .catch(console.error);
      channelToDelete.delete("Demandé par " + msg.author.tag + " " + msg.author.username).catch(console.error);
    }
    msg.channel
      .send(
