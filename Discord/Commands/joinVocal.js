...
module.exports = async function joinVocal(
  ...
  if (!msg.member.voice.channelId)
    ...
    [(await msg.member.fetch()).roles.cache.keys()].includes(
      process.env.ROLE_ETUDIANT_ID
    )
  ) {
    if (
    ...
    ![(await msg.member.fetch()).roles.cache.keys()].includes(
      process.env.ROLE_VACANCES_ENSEIGNANT_ID
    )
  ) {
    msg
      .reply(
    ...
    msg.guild.channels
      ...
      .then((channel) => {
        ...
        channel.permissionOverwrites.edit(
          process.env.ROLE_ENSEIGNANT_ID,
          discordUtils.permissionsLireEcrireProfOverwrite(true)
        );
        if (msg.mentions.roles.first()) {
          channel.permissionOverwrites.edit(
    ...
    (await msg.guild.channels.resolve(process.env.CHANNEL_ADMIN_ID))
      .send(
        ` :octagonal_sign: Alerte ! ${discordUtils.getUsername(msg.author)} / ${
          msg.member.nickname
