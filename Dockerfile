...
ARG DISCORD_CHAT_EXPORTER_VERSION="2.34.1"
ARG timezone=Etc/UTC

ENV NODE_VERSION=${NODE_VERSION}
ENV DISCORD_CHAT_EXPORTER_VERSION=${DISCORD_CHAT_EXPORTER_VERSION}
RUN apt-get update && apt-get install -y apt-transport-https zip && wget -qO- https://packages.microsoft.com/keys/microsoft.asc | gpg --dearmor > microsoft-prod.gpg && \
    ...
    chown root:root /etc/apt/sources.list.d/microsoft-prod.list && apt-get update && apt-get install -y dotnet-runtime-6.0 && apt-get autoremove
WORKDIR /usr/src
